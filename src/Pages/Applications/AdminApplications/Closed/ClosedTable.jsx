...
import ClosedData from "../Closed/ClosedData";
import Pagination from '../Pagination/Pagination';
import { useState } from "react";


const ClosedTable = ({ value }) => {
  const [currentPages, setCurrentPage] = useState(1);
  const [postPerPage] = useState(12);

  const indexOfLast = postPerPage * currentPages;
  const indexOfFirst = indexOfLast - postPerPage;
  const currentPost = ClosedData.slice(indexOfFirst, indexOfLast);
  const paginate = (pageNumber) => setCurrentPage(pageNumber);
  const page = Math.ceil(ClosedData.length / postPerPage);
